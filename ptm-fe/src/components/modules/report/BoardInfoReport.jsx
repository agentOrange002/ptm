...
class BoardInfoReport extends Component {
    ...
    getPDF = async () => {
		...
		await Axios.get(`http://localhost:8080/ptm/api/reports/board/${this.state.search}`, {
			responseType: 'blob',
			headers: {
    ...
    render() { 
        ...
        <div className="layout-main-inside">
			<h5>Board Info Report</h5>
                <span className="p-input-icon-left">
                    <i className="pi pi-search" />
                    <InputText defaultValue={this.state.search} onChange={(e) => this.setState({search:e.target.value})} placeholder="Enter Board ID:" />
                </span>
				<Button 
...
export default connect(mapStateToProps, null)(BoardInfoReport);